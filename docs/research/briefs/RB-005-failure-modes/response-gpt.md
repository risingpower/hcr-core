...
| Token selection | **Budget packing fails on “need-many-pieces” questions**: sub-400 tokens cannot support aggregation/listing tasks even with perfect retrieval | “List all exceptions / all projects / all vendors”; “what changed between versions”; these require broad evidence | Catastrophic (cannot answer) | 5–20% depending on how Su is used (agentic ops tends to ask more of these) | Submodular relevance–redundancy selection helps avoid wasted redundancy citeturn36search4turn36search0 | Residual is fundamental: if the answer requires many distinct facts, a 400-token cap sets a hard ceiling regardless of routing |
...
| Cross-branch subtype | Likely frequency in Su-like org KB | Layer 1: decomposition | Layer 2: soft assignment | Layer 3: entity cross-links | Layer 4: beam search | Layer 5: collapsed-tree fallback | Expected recall after all layers (best estimate) | Key residual risk |
|---|---:|---|---|---|---|---|---:|---|
...
### Implications for Su’s likely query mix (single-branch vs cross-branch)
...
- However, enterprise work-task studies describe a “tracing” technique relying on **historical and contextual relationships as paths**, which is essentially cross-branch traversal behaviour even when the interface is a single search box. citeturn25view3turn25view1  
  Su, as an agentic command centre (not a passive search box), is likely to *increase* the proportion of multi-step queries because agents naturally ask “connective” questions that compress multiple human steps into one instruction.

A conservative, decision-relevant estimate for Su-like usage therefore looks like:

- **Single-branch**: ~45–70% (higher if Su is used mostly for lookup/refinding). citeturn26view0turn27search6  
- **Cross-branch**: ~20–45% (higher if Su is used for planning, audits, project coordination, and “what changed / what’s impacted?”). citeturn25view3turn27search1  
...
Combined, if Su workload resembles “enterprise search logs” (mostly lookup), overall failure might be bounded in the low teens. If Su workload resembles “agentic ops copilot” (more aggregation/temporal/comparative), overall failure could drift into the 20–35% range unless Su is allowed to **multi-turn retrieve** or **call tools** (which is outside this architectural brief but affects feasibility). citeturn25view3turn27search1

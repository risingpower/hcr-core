...
**Date:** 2026-02-13
**Status:** Complete (4/4 sources)
**Brief:** [RB-004 Prompt](./prompt.md)
**Sources:** GPT, Gemini, Claude, Perplexity

## Summary

Four sources independently analysed tree construction for hierarchical retrieval. The convergence is exceptionally strong — all four arrive at the same recommended architecture through complementary analytical routes: **top-down divisive clustering (embedding-based) as the tree backbone, with LLM-generated structured contrastive summaries at each node, controlled soft assignment (1–3 parents per leaf) for multi-topic content, and incremental maintenance via PERCH/GRINCH-style local repairs between periodic full rebuilds.** The most important finding is that no construction method solves the cross-branch failure mode alone — the emerging best practice is a layered defense combining content decomposition, soft clustering, graph-based cross-links, and beam search at query time. A second critical finding is that **routing summaries are fundamentally different from reading summaries** — they should be structured artifacts (entities + keywords + contrastive boundaries + narrative) optimised for sibling differentiation, not comprehension. Gemini's contribution reinforces this with the strongest framing: summaries must be "discriminatory, machine-readable routing apparatus[es]", not human-readable prose, and provides the Q-STRUM Debate methodology as a concrete adversarial generation technique. Gemini also uniquely contributes **depth-variable summary formatting** — Level 1 should prioritise contrastive prose for macro-disambiguation, Level 2 should shift to keyword arrays + multi-vector embeddings for entity matching. For tree quality evaluation, **no routing-specific metric exists in the literature** — this is a genuine research gap that HCR can fill with per-level routing accuracy (analogous to selective search's Rn metric), sibling distinctiveness scores, and Schema Entropy Bound evaluation (Gemini). The federated search literature (CORI, ReDDE, query-based sampling) and metadata-augmented indexing (CHORUS, surfaced by Gemini) provide the framework for HCR's external-pointer leaf constraint.

---

## Consensus

All four sources agree on the following. Confidence reflects strength and independence of corroboration.

| # | Finding | GPT | Gemini | Perplexity | Claude | Confidence |
|---|---------|-----|--------|------------|--------|------------|
| 1 | **Top-down divisive clustering is the best backbone for HCR's constraints.** It naturally controls depth (d=2–3) and branching (b=8–12), produces near-balanced trees, and is computationally cheap. Bisecting k-means outperforms agglomerative methods for cluster quality while being O(n) vs O(n²). | Yes — bisecting k-means produces "more uniformly sized clusters"; split policies are the key lever | Yes — "divisive methods offer compelling computational and topological advantages"; explicitly parameterizable to force balanced branching; ~O(n log n) | Yes — "most natural backbone for HCR"; explicitly recommends top-down divisive with LLM-suggested facets at level 1 | Yes — bisecting k-means "produces better clusterings"; Bonsai (K=100, depth 1–2) outperforms deep binary trees | **Very High** |
| 2 | **RAPTOR's collapsed tree outperforming strict traversal is a structural prediction, not an anomaly.** The tree's value lies in creating multi-granularity representations (enrichment), not in enforcing a routing path. All sources derive this from first principles. | Yes — "the value of the tree may be in producing multi-granularity, differentiable summaries—not in enforcing a single routing path" | Implicit — frames tree as "routing apparatus" with summaries as "discriminatory machine-readable" artifacts; the tree's role is routing signal generation, not comprehension | Yes — collapsed tree "outperforms strict top-down traversal, suggesting the tree is more a multi-granularity representation than a reliable routing structure" | Yes — "tree construction and traversal strategy are not independent"; tree's value may be "more in organizing summarization than in providing a routing structure" | **Very High** |
| 3 | **Routing summaries must be structured, contrastive, and entity-preserving — not reading summaries.** Summaries should include: discriminative hooks (rare entities/IDs), contrastive boundary cues ("covers X, NOT Y"), multi-aspect coverage, and structured fields alongside prose. | Yes — "discriminative hooks, contrastive boundary cues, multi-aspect coverage, match-preserving representation" as four required fields | Yes — strongest framing: "must not be designed for human reading comprehension; must be explicitly engineered as a discriminatory, machine-readable routing apparatus." Three-layer format: keyword/entity array, contrastive boundary statement, brief narrative. Cites Q-STRUM Debate methodology. | Yes — structured schema: `{theme, includes, excludes, key_entities, key_terms, typical_queries}` | Yes — "contrastive summarization" theoretically well-motivated; "entity and identifier preservation" has strong evidence; structured formats show moderate evidence of advantage | **Very High** |
| 4 | **No single method solves cross-branch queries.** The emerging best practice is a layered defense: (1) content decomposition into atomic units, (2) soft clustering / leaf duplication, (3) graph-based cross-links, (4) beam search at query time, (5) collapsed-tree fallback. | Yes — four structural mitigations (soft assignment, graph augmentation, decomposition, multi-path); "cross-branch safety valve" required | Yes — covers decomposition, soft clustering, DAG structures, and query-time multi-path expansion; warns soft clustering "introduces cascading architectural inefficiencies" (summary dilution, write amplification) | Yes — five-layer defense: decomposition, controlled overlap, cross-links, beam search, HIRO-style thresholds | Yes — five layers: proposition decomposition, soft clustering, entity index, beam search (width 3–5), collapsed-tree fallback | **Very High** |
| 5 | **Standard cluster metrics (silhouette, Davies-Bouldin) are poor predictors of routing quality.** No routing-specific tree quality metric exists in the literature. This is a genuine research gap. | Yes — relevance-neighbourhood diagnostics and TPM are the closest; three routing-native metrics proposed | Yes — "near-zero predictive validity for top-down routing algorithms." Proposes Sibling Distinctiveness (SD) via BM25 softmax, Term Coherency (TC) via mutual information, and Schema Entropy Bound via Shannon entropy H(p). | Yes — Dasgupta cost adapted to query-relevance graph; per-level routing accuracy; sibling distinctiveness; coverage@k | Yes — selective search Rn metric is the closest analog; Optimum Clustering Framework (Fuhr 2012) is the principled direction | **Very High** |
| 6 | **LLMs provide highest value in summarization, less in partitioning.** Summarization is indispensable (proven by RAPTOR, LATTICE). Entity extraction is valuable but expensive. Partitioning can be safely left to embedding-based methods. LLM validation/refinement of summaries is promising but unproven. | Yes — LLMs on cluster representatives (not every document); hybrid pipelines proportional to number of base clusters | Yes — optimal integration is "semantic routing and partitioning judge" (LATTICE-style); LLM for boundary evaluation and Q-STRUM debate summarization restricted to top two levels to cap token spend | Yes — LLMs for summaries/labels/aspect suggestion; embeddings for bulk clustering and incremental updates | Yes — summarization is highest-value; semantic partitioning shows "minimal LLM value-add"; contrastive refinement is promising but unproven | **Very High** |
| 7 | **Near-balanced trees are preferred for routing.** Size imbalance creates "miscellaneous" branches with weak summaries and high sibling overlap. Bisecting k-means and PERCH both target balance. Variable branching within bounds (b ∈ [6, 15]) is better than fixed branching everywhere. | Yes — "enforce balance constraints during construction and maintenance, even if it slightly worsens within-cluster cohesion" | Yes — unbalanced trees are "structurally detrimental"; node summarising 50 children exceeds token budget; enforces strict min/max cluster size bounds | Yes — "near-balanced trees are preferred: they minimize average depth and spread misrouting risk more evenly" | Yes — "allow moderate imbalance" with soft constraints (min 3, max 3× median); variable branching ∈ [6, 15] | **Very High** |
| 8 | **Incremental maintenance follows "insert locally, repair locally, rebuild periodically."** PERCH (tree rotations for purity/balance), GRINCH (rotate + graft), and BIRCH (CF-tree summary statistics) provide transferable primitives. No system addresses routing-specific degradation detection. | Yes — PERCH, BIRCH, CluStream as maintenance primitives; "micro-branches" for new content | Yes — LSH hyperplane preservation for deterministic routing of new documents; localized upward-propagating summary updates; lazy-evaluation daemon monitoring node density and SD scores | Yes — PERCH rotations, OHAC split-merge, BERTopic incremental updates; per-node quality monitors with threshold-triggered subtree rebuilds | Yes — GRINCH rotate+graft, BIRCH CF-trees, split/merge heuristics; dirty-flag summary staleness tracking | **Very High** |
| 9 | **Multi-vector (ColBERT-style) representations at the summary level are strongly recommended.** They preserve detail hooks that single-vector embeddings wash out. MUVERA achieves 95% recall with reduced storage. Token pooling reduces ColBERT indexes by 50–75% with <5% degradation. | Yes — "store multi-vector representations for summaries so detail queries can still match rare tokens" | Yes — "single dense vector forces disparate, orthogonal concepts into a conflicted, geometrically averaged representation"; ColBERT MaxSim preserves "absolute geometric fidelity"; token pooling reduces vectors 50–75% with "virtually zero degradation" | Yes — "small set (e.g., 8–16) of multi-vectors from entity names, schema identifiers, and rare terms" | Yes — "high-confidence recommendation"; "directly address [DPI failure] by preserving token-level discriminative signals" | **Very High** (upgraded from High — 4/4 consensus) |
| 10 | **Content decomposition into atomic semantic units is the primary defense against cross-branch failure.** PropRAG achieves 94% Recall@5 on HotpotQA via proposition-based decomposition. Finer-grained units produce cleaner partitions with fewer multi-topic items. | Implicit — decomposition as "third mitigation"; route to source+offset rather than monolithic document | Yes — "LLMs to chunk the document into highly granular, semantically homogeneous segments prior to index construction"; merge-and-split for heterogeneous clusters | Yes — "primary defense": normalize to atomic units (sections, API endpoints, tickets); only irreducible multi-topic units need soft assignment | Yes — PropRAG explicitly cited; 94% Recall@5 on HotpotQA; "quality degrades significantly with weaker LLMs" | **High** |
| 11 | **Federated search (CORI, ReDDE) provides the framework for external pointer leaves.** 300–500 sampled documents per source are sufficient for effective resource selection. Imperfect representations work surprisingly well for routing. | Yes — BIRCH CF-tree concept: "smallest descriptor that still preserves routing hooks"; streaming clustering for drift | Yes — metadata-augmented indexing (CHORUS); federated synthesis with KL-divergence drift detection on sampled external data | Yes — federated search patterns: metadata + sampled content + historical queries; placeholder leaves for unavailable sources; ConStruM as direct analog | Yes — CORI robust to incomplete representations; query-based sampling; selective search (shard selection) as direct analog to HCR traversal | **High** |
| 12 | **Soft assignment is unusually cheap in HCR because leaves are pointers.** Duplicating a pointer across 2–3 branches incurs minimal storage overhead while dramatically reducing cross-branch elimination probability. | Yes — "duplicating a pointer across two branches incurs minimal storage, but greatly reduces the probability of cross-branch elimination" | Yes — but warns of "cascading architectural inefficiencies": duplicated content inflates token volume of summaries at every level above, plus consistency hazards on update (write amplification) | Yes — "controlled duplication at the leaf level: each leaf can have up to 2–3 parents" | Yes — "allowing leaves to have 1–3 parents captures cross-topic content without exploding storage" | **High** |
| 13 | **RAPTOR's GMM soft clustering is limited in practice.** Despite being the only mainstream system with native multi-membership, a Stanford CS224N analysis found GMM often produces flat structures with minimal actual soft clustering for standard document sizes. The Gaussian assumption is a poor fit for text embedding distributions. | Implicit — soft clustering is structurally important but RAPTOR's recursive clustering produces variable-depth, uneven branching | Not explicitly stated (but implicitly supported — frames agglomerative/GMM as producing uncontrolled depth and branching) | Not explicitly stated | Yes — Stanford CS224N analysis; GMM achieves only 55.17% accuracy as baseline; "Gaussian assumption is a poor fit for text embedding distributions" | **Medium** |
...
|---|-------------------|-----------|-----------|------------|
| 1 | **Bottom-up vs top-down as primary construction method** | GPT, Gemini, and Perplexity all recommend top-down divisive clustering as the primary backbone, citing controllable topology and natural alignment with HCR's d/b constraints. Gemini explicitly frames agglomerative methods as having "severe architectural limitations" for shallow-depth systems. | Claude recommends "proposition-based decomposition followed by embedding-based soft clustering (GMM or graph-community detection)" — a more bottom-up framing. | **Top-down divisive is the clear consensus (3/4 sources).** Bottom-up approaches (RAPTOR-style) produce uncontrolled depth and branching. Top-down divisive naturally constrains to d=2–3, b=8–12 and tends toward balanced trees. Claude's recommendation for GMM is weakened by their own finding that GMM is limited in practice. **Default to top-down divisive with soft assignment as a secondary mechanism for multi-topic leaves.** |
| 2 | **GraphRAG as primary vs secondary index** | Perplexity frames graph community detection as a "secondary index for cross-cutting, entity-heavy queries" — supplementary to the main tree. | Claude and GPT both acknowledge graph augmentation's value but don't frame the relationship as primary/secondary explicitly. | **Perplexity's framing is the most practical for HCR.** Full GraphRAG-style construction is 10–50× more expensive than RAPTOR. For this use case, a lightweight entity index (NLP-based extraction, not full LLM graph construction) as a cross-branch link structure is the right cost-quality tradeoff. Graph communities as a secondary index for entity-heavy global queries. |
...
| 6 | **Soft assignment: cheap & beneficial vs architecturally risky** | GPT, Perplexity, and Claude all frame soft assignment (1–3 parents per leaf pointer) as low-cost and high-benefit, since leaves are pointers and duplication is cheap. | Gemini warns that soft clustering "introduces cascading architectural inefficiencies": duplicated content inflates summary token volume at every ancestor level, and updates require tracking across multiple disconnected subtrees (write amplification and consistency hazards). Gemini prefers content decomposition + query-time beam search over pointer duplication. | **Both perspectives are valid at different scales.** At HCR's target of 1–3 parents per leaf, summary dilution is bounded (each parent summarises at most ~15% more content). Write amplification is real but manageable with pointer-based leaves (updating a pointer is O(1) per parent). Gemini's concern is more relevant if soft assignment escalates beyond 3 parents. **Keep the 1–3 parent constraint strict; Gemini's warning reinforces the ceiling.** |
| 7 | **LSH hyperplanes vs bisecting k-means for partitioning** | GPT, Perplexity, and Claude converge on bisecting k-means as the primary partitioning algorithm. | Gemini champions LSH hyperplane-based partitioning (EraRAG) as superior for HCR: deterministic bucket assignment, absolute control over topology via merge-and-split, localized maintenance (saved hyperplanes enable instant routing of new documents), and lower computational cost than k-means. | **Bisecting k-means remains the default recommendation** — it has more empirical support for semantic clustering quality and is the proven choice across RAPTOR, LATTICE, and selective search. However, **EraRAG's LSH approach is a valuable alternative worth noting for Phase 1 evaluation**, particularly for its deterministic incremental maintenance properties. The merge-and-split mechanism for enforcing branching factor constraints is a practical technique regardless of base partitioning method. |
...
### Between sources
- **All four sources now available.** Gemini confirms the three-source consensus on all major findings. No prior conclusions need revision.
- **Gemini uniquely surfaced** Q-STRUM Debate methodology (ACL Findings 2025) for adversarial contrastive summary generation, EraRAG's LSH hyperplane-based construction and deterministic incremental maintenance, CHORUS for metadata-augmented hierarchical retrieval without full-text access, VersionRAG for incremental upsert on evolving documents, CoRel's automated Sibling Distinctiveness metric (BM25 softmax over siblings), the Schema Entropy Bound theorem (information-theoretic tree quality evaluation via Shannon entropy), and LightRAG's lazy-evaluation daemon for background rebalancing. Gemini also provided the most detailed analysis of DAG-based index structures and their complications for cascade scoring (path recombination, multi-path convergence inflation). Gemini's strongest unique contribution is the **depth-variable summary formatting** recommendation: Level 1 summaries should prioritise contrastive prose for macro-disambiguation, while Level 2 summaries should shift to high-density keyword arrays + multi-vector embeddings for entity matching.
...
### 1. The construction recipe for HCR is convergent across all four sources
...
- **Contrastive cues**: generated with sibling context in the LLM prompt ("state what this branch covers and what it does NOT, relative to siblings"). Gemini proposes Q-STRUM Debate (ACL Findings 2025) — adversarial multi-persona argumentation to synthesise contrastive boundaries — as a concrete generation technique.
...
- **Depth-variable formatting** (Gemini): Level 1 summaries should prioritise contrastive prose for macro-disambiguation; Level 2 summaries should shift to high-density keyword arrays + pruned multi-vector embeddings for entity matching. This variable allocation maximises token budget utility.
...
Four proposed routing-native metrics (not yet validated), now with four-source input:
...
- **Sibling distinctiveness (SD)**: pairwise distance/divergence between sibling summaries; penalise hook overlap across siblings. Gemini provides a concrete formulation: BM25 softmax of class-indicative terms across siblings (CoRel methodology). SD approaching 1.0 = clear, unambiguous routing signal.
- **Term Coherency (TC)** (Gemini): mutual information of terms within a single node. High TC + high SD = optimal partition (internally unified, externally isolated).
- **Schema Entropy Bound** (Gemini): information-theoretic evaluation via Shannon entropy H(p) of semantic concept distribution. Uniform distribution = poor tree (maximum uncertainty); skewed distribution = good tree (concentrated concepts). Can pinpoint which depth level fails to transmit sufficient routing signal.
...
The deepest insight from all four sources: **optimising tree topology yields diminishing returns compared to optimising summary quality and traversal strategy**. RAPTOR's collapsed tree outperforms its strict traversal mode. LATTICE's LLM-guided traversal outperforms embedding-based traversal on the same tree. Gemini reinforces this by framing summaries as "the highest-leverage architectural intervention" — no downstream cross-encoder sophistication compensates for a summary that has structurally erased the entities required for a routing match. The implication: invest engineering effort in summary generation and beam-search traversal before fine-tuning clustering algorithms.
...
Based on four-source consensus:
...
2. **RB-005 (Failure modes)** — cross-branch queries remain the #1 structural risk. Quantify: what fraction of real queries in the target domain are cross-branch? What is the recall floor under beam search (k=3, d=2) with this construction strategy?
